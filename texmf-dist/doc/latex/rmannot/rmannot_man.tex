...
\pkg{AeB Pro} and \textsf{rmannot}.\footnote{Articles that use \FT{FLV} or
...
\begin{enumerate}
   \item \pkg{AeB} (\AcroTeX{} eDucation Bundle) The most recent version.
      In particular the \pkg{eforms} package and its companion
      package \pkg{insdljs}. The \pkg{AeB Pro} package is recommended. (All
      the demo files use \pkg{AeB Pro}.) Get it at \href{http://ctan.org/pkg/acrotex}{ctan.org/pkg/acrotex}.
  \item The \pkg{graphicxsp} package. The latest version, I made
...
\subsubsection{\tops{\protect\pkg}{}{rmannot} and non-Distiller workflows}

With \pkg{rmannot} dated 2021/04/21 or later, a non-Distiller workflow can be
utilized to compile and build a PDF; however, any Rich Media annotation
created by \cs{rmAnnot} is non-functional and is represented by a frame box
with the caption \textsf{Distiller required}. This allows the document author
to use his/her favorite PDF creator and favorite PDF previewer to develop the
document. To build the document to be published, \app{Acrobat/Distiller} are
required at the tail end of the workflow \textsf{tex -> dvi -> ps -> pdf}.
...
recognized, for those in the Beta Program of \app{Acrobat}.} The default is
\cs{AcroVer\darg{DC}}, which is equivalent to \cs{AcroVer[win=32]\darg{DC}}.
